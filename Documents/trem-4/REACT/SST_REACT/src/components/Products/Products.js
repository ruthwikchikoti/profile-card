...
function Products({ cart, incrementcart, DecrementCart}) {
    // const products = [
    //       {
    //         id: 1,
          ...
          <div><Effect /></div>
            {
                products.map(function (item, index) {
                  return (<ProductCard key={index}  product ={item} cart={cart} incrementCart={incrementcart} DecrementCart={ DecrementCart}  />)
                })
            }
        </div>
    )
